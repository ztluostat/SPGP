### Functions for estimation using NNGP ### 

if (.Platform$OS.type == "windows") {
  dyn.load("./BRISC/BRISC.dll")
} else {
  dyn.load("./BRISC/BRISC.so")
}
stopifnot(is.loaded("BRISC_estimatecpp"))

#' GP covariance parameter estimation using NNGP
#'
#' @param coords 
#' @param y 
#' @param x 
#' @param sigma.sq spatial variance
#' @param tau.sq nugget variance
#' @param phi range parameter in numerator such that phi*distance is the scaled distance
#' @param nu 
#' @param n.neighbors 
#' @param n_omp 
#' @param prior 
...
#'
#' @examples
NNGP_estimation <- function(coords, y, x = NULL, sigma.sq = 1, tau.sq = 0.1, phi = 1, nu = 1.5, n.neighbors = 15, n_omp = 1,
                            prior = c("a_phi" = 2, "b_phi" = 2, "a_alpha" = 2, "b_alpha" = 2, "a_sigma" = 2, "b_sigma" = 2),
...
Please check the new documentation with ?BRISC_estimation.')
  ...
  alpha.sq.starting <- sqrt(sigma.sq/tau.sq)
  ...
  ##estimtion
  result <- .Call("BRISC_estimatecpp", y, X, p, n, n.neighbors, coords, cov.model.indx, 
                  alpha.sq.starting, phi.starting, nu.starting, search.type.indx, n.omp.threads, verbose, eps, fix_nugget,
                  prior)
  ...
  if(cov.model!="matern"){
    names(Theta) <- c("alpha.sq", "phi")
  }
  else{names(Theta) <- c("alpha.sq", "phi", "nu")}
  ...
  result_list$log_post = result$log_post
...
# function to evaluate quadratic terms based on NNGP approximation
evalQuadNNGP <- function(B_mat, F_mat, u, v, nn_indx, nn_indx_LU) {
  # type conversion
  storage.mode(B_mat) <- "double"
  storage.mode(F_mat) <- "double"
  storage.mode(u) <- "double"
  storage.mode(v) <- "double"
  storage.mode(nn_indx) <- "integer"
  storage.mode(nn_indx_LU) <- "integer"
  
  quad = .Call("evalQuadCpp", B_mat, F_mat, u, v, nn_indx, nn_indx_LU)
  return(quad)
}
