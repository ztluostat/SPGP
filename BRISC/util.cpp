...
std::string getCorName(int i){
  ...
  if (i == 0) {
    ...
    return "matern52";  // matern covariance with smoothness = 5/2
  }else{
...
// correlation matrix (without nugget)
double spCor(double &D, double &phi, double &nu, int &covModel, double *bk){
  ...
  } else if (covModel == 4) { // matern with nu = 5/2
    return (1.0 + sqrt(5.0) * D*phi + 5.0/3.0 * D*phi * D*phi) * exp(-sqrt(5.0) * D*phi);
  } else{
    error("c++ error: cov.model is not correctly specified");
  }
}

//Description: computes the quadratic term: t(u) %*% t(I - B) %*% inv(F) %*% (I - B) %*% v
...
// density for inverse gamma distribution
double dinvgamma(double x, double shape, double rate, int give_log) {
  double log_density = dgamma(1.0 / x, shape, 1.0 / rate, 1) - 2.0 * log(x);
  if (give_log) return log_density;
  return exp(log_density);
}

// density for half-t distribution
double dtplus(double x, double df, double scale, int give_log) {
  double log_density = dt(x / scale, df, 1) + M_LN2 - log(scale);
  if (give_log) return log_density;
  return exp(log_density);
}
