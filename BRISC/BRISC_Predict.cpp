...
#pragma omp parallel for private(threadID, phi, nu, sigmaSq, tauSq, k, l, d, info)
    ...
    for(i = 0; i < q; i++) {
      ...
      // C: covariance matrix of neighbors (including nugget)
      for (k = 0; k < m; k++) {
      ...
      for (k = 0; k < m; k++) {
        ...
        d += tmp_m[threadID*m+k]*(y[nnIndx0[i+q*k]]);
      }
      ...
      y0[i] = d;
      // kriging variance
      vary0[i] = std::max(0.0, sigmaSq + tauSq - F77_NAME(ddot)(&m, &tmp_m[threadID*m], &inc, &c[threadID*m], &inc));
    }
    ...
    SET_VECTOR_ELT(resultName_r, 0, mkChar("kmean"));

    SET_VECTOR_ELT(result_r, 1, vary0_r);
    SET_VECTOR_ELT(resultName_r, 1, mkChar("kvar"));
